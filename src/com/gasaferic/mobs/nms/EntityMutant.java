...
public class EntityMutant extends EntityIronGolem {
	...
	public EntityMutant(World world) {
		...
		this.getBukkitEntity().setCustomName("§cMutante");
	...
	public void N() {
		this.goalSelector.a(6, new PathfinderGoalMeleeAttack(this, EntityHuman.class, 1.0D, true));
	...
	public IronGolem spawn(Location location) {
		World world = (World) ((CraftWorld) location.getWorld()).getHandle();
		EntityMutant customGolem = new EntityMutant(world);
		
		customGolem.setPosition(location.getX(), location.getY(), location.getZ());
		((CraftLivingEntity) customGolem.getBukkitEntity()).setRemoveWhenFarAway(false);
		world.addEntity(customGolem, SpawnReason.CUSTOM);
		
		return (IronGolem) customGolem.getBukkitEntity();
