...
import com.gasaferic.mobs.nms.EntityMutant;

import net.minecraft.server.v1_8_R3.World;

public class MutantSpawner implements Listener {
	
	final int RESPAWN_TIME;

	public MutantSpawner(ArrayList<Location> spawnersLocations, int respawn_time) {
		this.RESPAWN_TIME = respawn_time;
		spawnersLocations.forEach((spawner) -> spawnMutant(spawner));
	}
		...
		if (e.getEntity() instanceof IronGolem) {
			if (e.getEntity().hasMetadata("spawnLoc")) {
				new BukkitRunnable() {
					@Override
					public void run() {
						spawnMutant(getSpawnLoc(e.getEntity()));
					}
	...
	public void spawnMutant(Location spawner) {
		World craftWorld = ((CraftWorld) spawner.getWorld()).getHandle();
		EntityMutant mutant = new EntityMutant(craftWorld);
		mutant.spawn(spawner.getWorld().getHighestBlockAt(spawner).getLocation()).setMetadata("spawnLoc",
