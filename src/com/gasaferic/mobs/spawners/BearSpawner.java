...
import com.gasaferic.mobs.nms.EntityBear;

import net.minecraft.server.v1_8_R3.World;

public class BearSpawner implements Listener {
	
	final int RESPAWN_TIME;

	public BearSpawner(ArrayList<Location> spawnersLocations, int respawn_time) {
		this.RESPAWN_TIME = respawn_time;
		spawnersLocations.forEach((spawner) -> spawnBear(spawner));
	}
		...
		if (e.getEntity() instanceof Cow) {
			if (e.getEntity().hasMetadata("spawnLoc")) {
				new BukkitRunnable() {
					@Override
					public void run() {
						spawnBear(getSpawnLoc(e.getEntity()));
					}
	...
	public void spawnBear(Location spawner) {
		World craftWorld = ((CraftWorld) spawner.getWorld()).getHandle();
		EntityBear bear = new EntityBear(craftWorld);
		bear.spawn(spawner.getWorld().getHighestBlockAt(spawner).getLocation()).setMetadata("spawnLoc",
